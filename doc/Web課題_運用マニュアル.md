...
※最新版では、提出完了メッセージを緑背景で表示し、曜日・体調・メンタルのラベル表示を追加しました。
...
※ 最新版では、担任画面に体調・メンタルラベルを追加し、検索やタイムライン表示を改善しました。  
「いいね」ボタンの押下により即時反映され、全履歴・検索結果・タイムラインの状態が自然に遷移します。  
また、検索や生徒名リンク押下時には、画面描画後に履歴欄へ自動スクロールします。
...
5. **既読処理の取り消し機能** （追加実装）
   ...
   既読処理取り消し後は、再度生徒用連絡帳画面で内容の編集や先生アカウント画面にて既読処理を行うことが可能です。
...
az webapp deploy \
  ...
  --type zip
```  
- `-g` : リソースグループ名  ※独自で定義したものであり、環境によって異なります
- `-n` : Webアプリ名（Azure上のApp Service名）  ※独自で定義したものであり、環境によって異なります
...
データ投入処理は `management/commands/seed_bulk.py` に定義されており、下記コマンドにより自動生成されます。
   ```bash
...
- 各機能の挙動が、開発環境での動作と同一であることを確認します。
